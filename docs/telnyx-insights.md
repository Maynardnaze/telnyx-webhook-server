...
The same `insight_id` values appear on every webhook from a given Insight Group. Map them to human-readable names in your Telnyx portal (for example: “Caller name”, “Sentiment”, “Lead summary”).
...
Example JSON-string result (sentiment / intent insight):
...
Example structured JSON result (caller name extraction):
...
Example markdown result (call categorization):
...
Example plain-text result (lead summary):
...
### Example A — phone call (completed transfer)

Telnyx POST body (abbreviated):
...
Same Insight Group, but `metadata` shows `sms_chat` and omits call fields:
...
3. **Build an `insight_id` → name map** from Mission Control once; IDs are stable per insight definition.
